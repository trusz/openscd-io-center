...
export let TreeNode = class extends LitElement {
  constructor() {
    super();
    this.path = "";
    this.node = void 0;
  ...
  renderChildren(isLeaf) {
    if (!isLeaf && this.expanded) {
      return html`
        <div class="children">
          ${this.node.children.filter((child) => !!child).map((child) => html`
            <tree-node
              .node="${child}"
...
], TreeNode.prototype, "level", 2);
__decorate([
  property({type: Boolean})
], TreeNode.prototype, "addedToEditor", 2);
__decorate([
  property({type: String})
], TreeNode.prototype, "searchTerm", 2);
__decorate([
  state()
], TreeNode.prototype, "isSelected", 2);
__decorate([
  state()
], TreeNode.prototype, "currentSelectedNode", 2);
TreeNode = __decorate([
