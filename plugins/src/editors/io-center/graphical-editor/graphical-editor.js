...
export let GraphicalEditor = class extends IoCenterFoundation {
  constructor() {
    super();
    this.rectangles = [];
  ...
  createConnection(from, to, inputId) {
    ...
    if (!this.connectionMap.has(id)) {
      this.connectionMap.set(id, void 0);
    } else {
      return;
    }
  ...
  isValidConnection(startType, target) {
    ...
    return startType === "output" && targetType === "input" || startType === "input" && targetType === "output";
  }
...
__decorate([
  state()
], GraphicalEditor.prototype, "rectangles", 2);
__decorate([
  state()
], GraphicalEditor.prototype, "connectionMap", 2);
