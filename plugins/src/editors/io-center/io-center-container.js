...
export let IoCenterContainer = class extends IoCenterFoundation {
  constructor() {
    super();
    this.currentIed = "";
    this.lpModels = [];
    this.currentSelectedRouter = void 0;
  ...
  generateRouterRectangleOptions(dataObject) {
    ...
    return {
      ...
      width: 200,
      height,
      inputConnectors: dataObject.connectors.filter((connector) => connector.type === "input").map((connector) => ({text: connector.name, position: "right", type: "input", id: connector.id})),
      outputConnectors: dataObject.connectors.filter((connector) => connector.type === "output").map((connector) => ({text: connector.name, position: "left", type: "output", id: connector.id})),
  ...
  saveEditor() {
    ...
    routers.forEach((router) => {
      ...
      router.connectors.forEach((connector) => {
        ...
        const targetConnector = targetDataObject.connectors.find((connector2) => connector2.id === targetConnectorId);
        if (!targetConnector) {
  ...
  processRouterLN(routerLN, lnRefDataObject) {
    ...
    doiElements.forEach((doi) => {
      ...
      lnRefElements.forEach((lnRef) => {
        ...
        if (refLDInst === lnRefDataObject.ldRef && refLNClass === lnRefDataObject.lnClass && refLNInst === lnRefDataObject.lnInst && refDO === lnRefDataObject.connectors[0].name) {
          connectionList.push({
            id: crypto.randomUUID(),
...
], IoCenterContainer.prototype, "currentIed", 2);
__decorate([
  query("graphical-editor")
], IoCenterContainer.prototype, "graphicalEditor", 2);
__decorate([
  query("tree-view")
], IoCenterContainer.prototype, "treeView", 2);
__decorate([
  query("lp-list")
], IoCenterContainer.prototype, "lpList", 2);
__decorate([
  state()
], IoCenterContainer.prototype, "lpModels", 2);
__decorate([
  state()
], IoCenterContainer.prototype, "currentSelectedRouter", 2);
__decorate([
  state()
], IoCenterContainer.prototype, "showModal", 2);
__decorate([
  state()
], IoCenterContainer.prototype, "isFormValid", 2);
